...
세 층 구조: **전체 업무(보드) ⊃ 업무(Task 조합) ⊃ Task(assistant 연계 단위)**

| 개념 | 정의 |
|---|---|
| **전체 업무 (워크플로우 보드)** | 모든 업무를 한 화면에서. 단계 유형(StepKey) 축으로 필터/칸반. |
| **업무** | 하나의 개발/변경 건. 코드(ET-2026-0031), 담당, 기한, 외부 시스템 참조를 가지며 **Task의 조합**이다. Task가 0개인 **수동 업무**도 가능(메모/첨부/이력만으로 진행). 구성 방식: 템플릿 / 모듈 직접 조합 / 수동. 진행 중에도 "워크플로우 구성"에서 Task를 갈아끼운다(추가·순서·제거). |
| **Task** | 업무 안의 실질 업무 단위. assistant와 연계(또는 수동), 체크리스트, 입력/산출물, 대화 스레드를 가진다. 상태 `pending → in_progress → done/skipped`, 언제든 `reopen`. 코드상 이름은 StepInstance. |
| **Task 모듈 라이브러리** | 재사용 가능한 Task 정의(키, 이름, 진행 방식, assistant 모델, 체크리스트, 입력/산출물). 템플릿 편집기·업무 화면에서 가져다 쓰고, 진행 중 Task를 라이브러리에 저장할 수 있다. |
| **워크플로우 템플릿** | 자주 쓰는 모듈 조합(순서 포함) + 템플릿 기본 assistant 모델. 업무 생성 시 복사되므로 이후 템플릿 변경은 새 업무에만 적용. |
...
### 실시간 협업 (데모 범위)
- **탭별 사용자**: 상단 사용자 전환은 탭(sessionStorage)에만 적용된다. 같은 브라우저에서 탭 A=박비오, 탭 B=이희준으로 같은 스레드에 동시에 참여할 수 있다.
- **데이터 동기화**: 모든 화면이 IndexedDB를 `liveQuery`로 구독하므로 메시지/체크/파일/상태 변경이 다른 탭에 즉시 반영된다. assistant 응답도 250ms 간격으로 DB에 flush되어 다른 탭에서 스트리밍처럼 보인다.
- **Presence**: 입력 중 표시는 BroadcastChannel로 탭 간 전달(DB에 남기지 않음). 다른 참여자의 요청에 assistant가 응답 중이면 입력창을 잠가 응답이 겹치지 않게 한다.
- **실서비스**: 서로 다른 PC 간 실시간은 백엔드가 필요하다 — 스레드 이벤트 채널(WebSocket/SSE), 서버 측 LLM 호출 + 토큰 브로드캐스트, 동시 요청 직렬화(스레드당 1개), presence 서비스. 화면 코드는 `useLiveQuery`를 서버 구독으로 바꾸면 된다.
