...
interface NotesPanelProps {
  ...
  notes: Note[]
  files: FileAsset[]
  /** true면 현재 단계 메모만, false면 업무 전체 메모 */
  scopeToStep?: boolean
  placeholder?: string
}

export function NotesPanel({ taskId, stepId, notes, files, scopeToStep = true, placeholder }: NotesPanelProps) {
  ...
  const visible = scopeToStep && stepId ? notes.filter((n) => n.stepInstanceId === stepId) : notes

  async function submit() {
    if (!actor || (!text.trim() && pending.length === 0)) return
    const uploaded = await Promise.all(pending.map((f) => uploadFile(actor, taskId, f, stepId)))
    await addNote(actor, taskId, text.trim() || `(첨부 ${uploaded.length}건)`, stepId, uploaded.map((f) => f.id))
    setText('')
    setPending([])
  }
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="min-h-0 flex-1 space-y-2 overflow-y-auto">
        {visible.length === 0 && <div className="rounded-lg border border-dashed p-4 text-center text-xs text-muted-foreground">메모가 없습니다.</div>}
        {visible.map((n) => {
